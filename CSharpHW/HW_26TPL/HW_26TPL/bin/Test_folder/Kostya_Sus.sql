...
--0.4
--1
SELECT EmployeeId AS ID, FirstName + ' ' +LastName AS NAME, YEAR(BirthDate) AS BirthYear FROM HR.Employees AS Employees; 

--2
DECLARE @date DATETIME = SYSDATETIME(); 
SELECT EOMONTH( @date ) AS Result; 
SELECT DATEADD(yy, DATEDIFF(yy,0,getdate()) + 1, -1) AS EndOfYear ;
SELECT DATEFROMPARTS ( YEAR(@date), 12, 31 ) AS Result; 

--3
SELECT Products.ProductId AS Result FROM Production.Products AS Products;
SELECT RIGHT(REPLICATE('0',10) + Cast(Products.ProductId AS VARCHAR),10) AS Result FROM Production.Products AS Products;


--0.5
--1
 SELECT OrderId AS ID_of_order FROM Sales.Orders WHERE ShippedDate IS NULL;
 --2
 SELECT OrderId AS Numder_of_order, OrderDate AS OrderDate FROM Sales.Orders WHERE OrderDate Between '2008-02-11' AND '2008-02-12';
 --1
 SELECT OrderId AS ID_of_order FROM Sales.Orders WHERE CustomerId =7 Order by ShipperID ASC,ShippedDate DESC;
 SELECT OrderId AS ID_of_order, ShipperId AS ShipperId ,ShippedDate AS Date_of_ship FROM Sales.Orders
  WHERE CustomerId =7 Order by ShipperID ASC,ShippedDate DESC;
 --2
 SELECT ProductId AS ID, ProductName AS Name, UnitPrice AS Price FROM Production.Products
  WHERE CategoryId = 1 ORDER BY Products.UnitPrice DESC OFFSET 0 ROWS FETCH NEXT 5 ROWS ONLY
 --3
 --ASC
  SELECT  ProductName AS Name, UnitPrice AS Price FROM Production.Products
...
--0.6
--1
 SELECT Sales.Orders.OrderId AS ID_ORDER, Sales.Customers.ContactName AS Name_Customer
 ...
 --5
 SELECT Sales.Orders.OrderId AS ID_ORDER, Sales.Customers.ContactName AS Name_Customer,Sales.Customers.CustomerId AS CustomerID,
...
DELETE Sales.MyCustomers FROM Sales.MyCustomers LEFT OUTER JOIN Sales.MyOrders ON Sales.MYCustomers.CustomerId = Sales.MyOrders.CustomerId
 ...
 --9
SELECT COUNT(CustomerId) FROM Sales.MyCustomers
--10
